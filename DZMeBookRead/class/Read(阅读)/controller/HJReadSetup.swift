...
class HJReadSetup: NSObject,UIGestureRecognizerDelegate,HJReadSettingColorViewDelegate,HJReadSettingFlipEffectViewDelegate,HJReadSettingFontViewDelegate,HJReadSettingFontSizeViewDelegate,HJReadLeftViewDelegate {
    ...
    func setFlipEffect(flipEffect: HJReadFlipEffect,chapterID:String,chapterLookPageClear:Bool,contentOffsetYClear:Bool) {
        ...
        if flipEffect == HJReadFlipEffect.None { // 无效果
            ...
            // 跳转章节
            readPageController.readConfigure.GoToReadChapter(chapterID,isInit: true,chapterLookPageClear: chapterLookPageClear, transitionStyle: UIPageViewControllerTransitionStyle.PageCurl)
            
        }else if flipEffect == HJReadFlipEffect.Translation { // 平滑
            
            // 跳转章节
            readPageController.readConfigure.GoToReadChapter(chapterID,isInit: true,chapterLookPageClear: chapterLookPageClear, transitionStyle: UIPageViewControllerTransitionStyle.Scroll)
    ...
    func updateFont() {
        ...
        // 重新展示
        readPageController.pageViewController.setViewControllers([readPageController.readConfigure.GetReadViewController(readPageController.readConfigure.readChapterModel, currentPage: readPageController.readModel.readRecord.page.integerValue)], direction: UIPageViewControllerNavigationDirection.Forward, animated: false, completion: nil)
    }
}
